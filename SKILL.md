...
#### 产品喵 10min cron 自检规则（v3，2026-05-25）
每 10 分钟运行 `/tmp/cron-pm-audit.py`，共 12 条规则，任一 FAIL → SendMessage 给 team-lead 附 "🚨 CORRUPTION DETECTED"：

| # | 规则 | 检测内容 |
|---|------|---------|
| 1 | 平账 | 待+进+完 = 总需求数 |
| 2 | toolbar | filter-count 各优先级数字 |
| 3 | stale | 已完成列不含 ⏳/⚠️ 图标 |
| 4 | duplicate | 卡号不重复 |
| 5 | corruption | `</div>>>` 出现次数 = 0 |
| A | req-desc 未转义 HTML | desc 内不含 `<div class="flex`/`<span class="inline-flex` 等业务 HTML（应被 escapeHtml 转义） |
| B | div 平衡（非 script 区） | HTML 主体 `<div` open count = `</div>` close count，diff ≤ 2 |
| C | column 数 | `<div class="column">` 正好 3 个 |
| D | 实际 pending 卡数 | DOM pending 列 req-card 数 == stat_待 |
| E | 实际 progress 卡数 | DOM progress 列 req-card 数 == stat_进 |
| F | column-count badge 待 | 待开发列头徽章数字 == stat_待 |
| G | column-count badge 进 | 进行中列头徽章数字 == stat_进 |

规则 A 背景：2026-05-26 CEO 在验收驳回弹窗贴了 DevTools `<div class="flex...">` 原始字符串，FSA 未 escape，破坏 3 列布局。已修 template `escapeHtml()`，此规则作为主动防线。
规则 D/E/F/G 背景：2026-05-25 FSA 验收驳回写回 [改进] 卡时未同步 stats/column-count/toolbar，导致 pending=4 而 stat_待=1。已修 template `patchStatsForNewPendingCard()`，cron 规则 D-G 作为持续监测。
...
### 1.5 卡片 class 规范（移卡时容易漏，必看）

不同列里的 `req-card` class 不一样，**移卡时必须同步改 class**，否则验收按钮等 CSS 行为会失效：

| 所在列 | 完整 class | 备注 |
|--------|-----------|------|
| 待开发 | `req-card` | 默认状态 |
| 进行中 | `req-card` | 默认状态（同待开发） |
| 已完成 | `req-card done` | **必须加 done** — `injectVerifyButtons()` 只挂按钮到 `.req-card.done`；不加 done 看板就没有「🔍 验收」按钮 |
| 归档 | `req-card done archived` | 用户验收通过后由前端 JS / FSA 自动写，产品喵不用手写 |

**实操**：用 Edit 工具移卡时，剪切节点 + 改 class 同步进行。从「进行中」移到「已完成」就把 `class="req-card"` 改成 `class="req-card done"`；反向移回（极少）就把 `done` 拿掉。
